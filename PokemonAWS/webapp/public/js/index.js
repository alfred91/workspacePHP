...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (token) {
    fetch("http://http://44.211.39.123:3000api/pokemon/list", {
      headers: {
      ...
      .then((pokemons) => {
        ...
        pokemons.forEach((pokemon) => {
          ...
          card.className = "card";
          card.innerHTML = `
            <img src="http://http://44.211.39.123:3000/images/${pokemon.imagen}" alt="${
            pokemon.nombre
          }">
...
document.getElementById("searchForm").onsubmit = async function (event) {
  ...
  try {
    const response = await fetch(
      `http://44.211.39.123:3000/api/pokemon/find/${nombre}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    if (pokemones.length === 0) {
      ...
      pokemones.forEach((pokemon) => {
        ...
        card.className = "card";
        card.innerHTML = `
            <img src="http://44.211.39.123:3000/images/${pokemon.imagen}" alt="${
          pokemon.nombre
        }">
...
document.getElementById("filterButton").onclick = async function () {
  ...
  try {
    const response = await fetch(
      `http://44.211.39.123:3000/api/pokemon/tipo/${selectedType}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    data.forEach((pokemon) => {
      ...
      card.className = "card";
      card.innerHTML = `
                <img src="http://44.211.39.123:3000/images/${
                  pokemon.imagen
                }" alt="${pokemon.nombre}">
                <h2>${pokemon.nombre}</h2>
                <ul class="list-disc pl-4">
...
function obtenerDetallesPokemon(pokemonId) {
  ...
  fetch(`http://44.211.39.123:3000/api/pokemon/id/${pokemonId}`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  })
...
function renderPokemonDetails(pokemon) {
  ...
  ? `<img src="http://44.211.39.123:3000/images/${pokemon.imagen}" alt="Imagen de ${pokemon.nombre}" class="pokemon-image">`
...
function borrarPokemon(id) {
  ...
  fetch(`http://44.211.39.123:3000/api/pokemon/delete/${id}`, {
    method: "DELETE",
    headers: {
