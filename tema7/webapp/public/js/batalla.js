  ...
  try {
    ...
    pokemons.forEach((pokemon) => {
      ...
      document.addEventListener("DOMContentLoaded", async function () {
        try {
          const response = await fetch(
            "http://localhost:3000/api/pokemon/list",
            {
              headers: { Authorization: `Bearer ${token}` },
            }
          );
      ...
      async function loadAbilities(
        pokemonId,
        abilitiesContainerId,
        opponentPokemonId
      ) {
        try {
          const response = await fetch(
            `http://localhost:3000/api/pokemon/id/${pokemonId}`,
            {
              headers: { Authorization: `Bearer ${token}` },
            }
          );
          ...
          const abilitiesContainer =
            document.getElementById(abilitiesContainerId);
      ...
      document
        .querySelector('select[name="pokemonId1"]')
        .addEventListener("change", function () {
          const pokemonId = this.value;
          loadAbilities(
            pokemonId,
            "abilitiesContainer1",
            pokemon2Data ? pokemon2Data._id : null
          );
        });

      document
        .querySelector('select[name="pokemonId2"]')
        .addEventListener("change", function () {
          const pokemonId = this.value;
          loadAbilities(
            pokemonId,
            "abilitiesContainer2",
            pokemon1Data ? pokemon1Data._id : null
          );
        });
      ...
      document
        ...
        .addEventListener("click", async function () {
          ...
          const pokemonId1 = document.querySelector(
            'select[name="pokemonId1"]'
          ).value;
          const pokemonId2 = document.querySelector(
            'select[name="pokemonId2"]'
          ).value;
          ...
          try {
            ...
            console.error(
              "Error al cargar los detalles de los Pokémon:",
              error
            );
          }
        });
      ...
      function updatePokemonModal(pokemon, modalId) {
        ...
        modal.querySelector(
          "img"
        ).src = `http://localhost:3000/images/${pokemon.imagen}`;
        modal.querySelector(".tipo").textContent = `Tipo: ${
          pokemon.tipo || "No disponible"
        }`;
        modal.querySelector(".especie").textContent = `Especie: ${
          pokemon.especie || "No disponible"
        }`;
        modal.querySelector(
          ".puntosSaludJuego"
        ).textContent = `Puntos de Salud en Juego: ${
          pokemon.puntosSaludJuego || "No disponible"
        }`;
      }

      async function atacarPokemon(
        pokemonAtacanteId,
        abilitiesContainerId,
        indexHabilidad,
        pokemonObjetivoId
        ...
        try {
          const habilidadSeleccionada =
            pokemon1Data._id === pokemonAtacanteId
          ...
          const attackResponse = await fetch(
            `http://localhost:3000/api/pokemon/${pokemonObjetivoId}/ataque/${puntosAtaque}`,
            {
              method: "PUT",
              headers: {
          ...
          if (!attackResponse.ok) {
            throw new Error(
              `Error en la respuesta del servidor al atacar: ${attackResponse.status}`
            );
          }
          ...
          const targetPokemonData =
            pokemonAtacanteId === pokemon1Data._id
              ? pokemon2Data
              : pokemon1Data;
          targetPokemonData.puntosSaludJuego =
            attackResult.pokemon.puntosSaludJuego;
...
async function atacarPokemon(
  ...
  try {
    ...
    const targetPokemonData =
      pokemonAtacanteId === pokemon1Data._id ? pokemon2Data : pokemon1Data;
